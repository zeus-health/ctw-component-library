...
const { TailwindTheme, CLASS_PREFIX, nameCSSVar } = require("./tailwind-gen.theme.cjs");

const createCSSVar = (name, defaultVal) => `var(${nameCSSVar(name)}, ${defaultVal})`;
...
const addCSSVarReference = (colorConfig) => {
  const config = Object.entries(colorConfig.colors).map(([colorTitle, colorValueOrObj]) => {
    if (typeof colorValueOrObj === "string") {
      return { [colorTitle]: createCSSVar(colorTitle, colorValueOrObj) };
    }
    const transformedColorArr = Object.entries(colorValueOrObj).map(([colorName, value]) => {
      return {
