...
import { getBuilderMedications, getPatientLensMedications } from "@/fhir/medications";
import type { MedicationBuilder } from "@/fhir/medications";

export function useQueryPatientMeds(statusParam = ""): UseQueryResult<MedicationBuilder, unknown> {
  const fhirClientRef = useFhirClientRef();
  const patient = usePatient();

  const queryKey = compact(["patient-medications", statusParam || null]).join('-');
  return useQuery(
    [queryKey, patient.data?.UPID, {
...
export function useQueryPatientLensMeds(): UseQueryResult<MedicationBuilder, unknown> {
  const fhirClientRef = useFhirClientRef();
  const patient = usePatient();
